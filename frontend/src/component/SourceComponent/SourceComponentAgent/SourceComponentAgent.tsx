...
import EditSourceAgent from "modal/Source/SourceAgent/EditSource";
...
import { startEditingSourceAgent } from "redux/Reducer/SourceReducerAgent";
import { showModal } from "redux/Redux/modal/modalSlice";
import { RootState, useAppDispatch } from "redux/store";

const SourceComponent: React.FC = () => {
    const dispatch = useAppDispatch();
    const data = useSelector((state: RootState) => state.sourceAgent.sourceList);
    ...
    const Agent = useSelector((state: RootState) => state.sourceAgent.sourceList);
    ...
    const filteredServices = Agent.filter(sourceAgent =>
        (sourceAgent.name && sourceAgent.name.toLowerCase().includes(searchQuery.toLowerCase()))
    );
        ...
        },
        {
            ...
            render: (text: string, record: { id: string }) => (
                <Button
                    ...
                    onClick={() => {
                        dispatch(startEditingSourceAgent(record.id));
    ...
    return (
        <div className="wrapper-layout-content">
            ...
            <EditSourceAgent visible={isEditModalVisible} onClose={handleEditModalClose} />
        </div>
    );
};
