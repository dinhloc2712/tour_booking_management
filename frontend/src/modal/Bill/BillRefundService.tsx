...
import { BillRefundServiceOK } from 'redux/API/POST/PostRefundBillService'
import { ThunkDispatch } from 'redux-thunk'
import { Action } from 'redux'
import { getAllUserCheckin } from 'redux/API/GET/getBooking/GetAllUserCheckin'
import {
  BillRefundServicePayload,
  ApiResponse,
  clearBillData,
...
const BillRefundService: React.FC = () => {
  ...
  //   console.log('checkInAllResult', checkInAllResult)
  // Sử dụng `checkInResult` để hiển thị hoặc tính toán
  //   console.log('Check-In Result for Bill:', checkInResult)

  const isOpen = useSelector(
    (state: RootState) => state.modal.modals['billModalRefund'] || false
  )
  const billData = useSelector(
    (state: RootState) => state.bill?.refundServiceBillData
  )
  //   console.log('billData', billData)

  const handleOk = () => {
    dispatch(hideModal('billModalRefund'))
    window.print()
  }
  ...
  const calculateTotalAmount = () => {
    ...
    billData?.refundServices?.forEach((service) => {
      // Tính tổng giá dịch vụ (price * quantity)
  ...
  const amount = 0 - totalAmount

  const handleSubmitBill = async () => {
    const totalAmount = calculateTotalAmount()
    const amount = 0 - totalAmount

    const BillRefundServicePayload: BillRefundServicePayload = {
      booking_id: checkInAllResult[0]?.tours[0]?.booking_id,
      customer_id: billData?.customerRefunds[0]?.id,
      staff_id: userId,
      name_staff: userName,
      voucher_id: null,
      code_voucher: null,
      value_voucher: null,
      quantity_customer: billData?.customerRefunds.length,
      ...
      refunds: {
        ...
        refundServices: billData?.refundServices.map((service) => ({
    ...
    // console.log('Bill payload data là :', BillRefundServicePayload)

    try {
      const result = await dispatch(
        BillRefundServiceOK(BillRefundServicePayload)
      ).unwrap()
  ...
  return (
    ...
    >
      <div className="invoice-container" id="invoice-container">
        ...
        {billData && (
          <div className="invoice-info">
            <p>
              ...
              {billData.customerRefunds[0]?.fullname}
            </p>
            <p>
              <label>Số điện thoại:</label>{' '}
              {billData.customerRefunds[0]?.user_detail.phone || ''}
            </p>
            <p>
              <label>Số lượng khách:</label> {billData.customerRefunds.length}
            </p>
          </div>
        )}
        {/* Loại hóa đơn */}
        ...
        <hr />
        {billData?.refundServices?.map((service, index) => (
          <div className="tour-block page-break" key={service.service.id}>
            {/* Dịch vụ của tour */}
            ...
            <table className="table">
              ...
              <tbody>
                {(() => {
                  ...
                  const groupedServices = billData?.refundServices.reduce(
                    (acc, service) => {
        ...
        <div className="invoice-summary page-break">
          <p>
            ...
            >
              <h5>{amount < 0 ? 'Tiền hoàn trả' : 'Tổng tiền'} :</h5>
              <h4 style={{ color: 'red' }}>
                {Math.abs(amount).toLocaleString()} VNĐ
              </h4>
            </div>
...
export default BillRefundService
